...
public interface ForumMapper {
    ...
    @Insert("insert into forum(title, content, author, time, reply_num, like_num, type) values(#{title}, #{content}, #{author}, #{time}, #{reply_num}, #{like_num}, #{type})")
    @Options(useGeneratedKeys = true, keyProperty = "id", keyColumn = "id")
    @Transactional
    int addForumItem(ForumItem forumItem);

    @Select("select forum.id id, title, content, nickname author_name, avatar_url author_avatar, time time, reply_num reply_num, like_num like_num, type from forum, user where forum.author = user.id order by time desc")
    ...
    @Select("select forum.id id, title, content, nickname author_name, avatar_url author_avatar, time time, reply_num reply_num, like_num like_num, type from forum, user where forum.author = user.id and forum.id = #{id}")
